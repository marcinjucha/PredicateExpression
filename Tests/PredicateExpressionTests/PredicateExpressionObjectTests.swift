...
final class PredicateExpressionObjectTests: XCTestCase {
    ...
    func test_Equal_WithName() {
        ...
        NSPredicate(nameKey == name)
            ...
            .verify(data.test, comparison: { $0.name == name })
    }
    ...
    func test_GreaterEqual_AgeGreaterEqual40() {
        NSPredicate(ageKey >= age)
            ...
            .verify(data.test, comparison: { $0.age >= age })
    }
    ...
    func test_GreaterEqual_AggregateAny_ChildrenAge() {
        ...
        NSPredicate(.array(childrenAgeKey >= age, aggregate: .any))
            ...
            .verify(data.test, comparison: { $0.children.filter { $0.age >= age }.count > 0 })
    ...
    func test_Between_AggregateAll_ChildrenAge() {
        ...
        NSPredicate(.between(childrenAgeKey, values: [lower, upper], aggregate: .all))
            ...
            .verify(data.test,
                    comparison: { $0.children.filter { $0.age < lower || $0.age > upper }.count == 0 })

    }

    func test_In_AggregateAll_ChildrenAge() {
        let ages = [7, 9, 10, 11, 13]

        NSPredicate(.in(childrenAgeKey, values: ages, aggregate: .all))
            .verify(predicate: predicate(key: childrenAgeKey, value: ages, comparison: .in, aggregate: .all))
            .verify(data.test,
                    comparison: { $0.children.filter { ages.contains($0.age) == false }.count == 0 })
    }

    func test_In_AggregateNone_ChildrenAge() {
        let ages = [10, 11, 13]

        NSPredicate(.in(childrenAgeKey, values: ages, aggregate: .none))
            .verify(predicate: predicate(key: childrenAgeKey, value: ages, comparison: .in, aggregate: .none))
            .verify(data.test,
                    comparison: { $0.children.filter { ages.contains($0.age) }.count == 0 })
    }
    ...
    func test_Less_AggregateNotAny_ChildrenAge() {
        ...
        NSPredicate(.array(childrenAgeKey < age, aggregate: .none))
            ...
            .verify(data.test,
                    comparison: { $0.children.filter { $0.age < age }.count == 0 })
    ...
    func test_BeginsWith_Name() {
        ...
        NSPredicate(.beginsWith(nameKey, value: name))
            ...
            .verify(data.test,
                    comparison: { $0.name.hasPrefix(prefix) })
    ...
    func test_Contains_Name() {
        ...
        NSPredicate(.contains(nameKey, value: namePart))
            ...
            .verify(data.test,
                    comparison: { $0.name.contains(namePart) })
    ...
    func test_EndsWith_Name() {
        ...
        NSPredicate(.endsWith(nameKey, value: suffix))
            ...
            .verify(data.test,
                    comparison: { $0.name.hasSuffix(suffix) })
    ...
    func test_Like_Name() {
        ...
        NSPredicate(.like(nameKey, value: name))
            ...
            .verify(data.test,
                    comparison: { $0.name[value: $0.name.count - 2] == char })
    ...
    func test_Matches_Name() {
        ...
        NSPredicate(.regex(nameKey, value: regex))
            ...
            .verify(data.test,
                    comparison: { $0.name.range(of: regex, options: .regularExpression) != nil })
    ...
    func test_And_EqualWithName_GreaterWithAge() {
        ...
        NSPredicate(nameKey == name && ageKey > age)
            ...
            .verify(data.test, comparison: { $0.name == name && $0.age > age })
    }
    ...
    func test_Not_EqualWithNameAndAge() {
        let name = femaleName
    ...
    func test_And_EqualWithName_Or_GreaterWithAge_LessPartnerWithAge() {
        ...
        NSPredicate(nameKey == name && (ageKey > age || partnerAgeKey < age))
            ...
            .verify(data.test, comparison: { $0.name == name && ($0.age > age || $0.partner.age < age) })
    }
